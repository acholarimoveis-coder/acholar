...
export default function PublicidadeManager({ anuncios }) {
  ...
  async function enviarImagem(e) {
    ...
    const nome = `banner-${Date.now()}-${arq.name.replace(/[^a-zA-Z0-9.\-_]/g, "")}`;
    const { error } = await supabase.storage.from("publicidade").upload(nome, arq, { upsert: true });
    if (error) { setErro("Erro ao enviar a imagem: " + error.message); setSubindo(false); return; }
    const { data } = supabase.storage.from("publicidade").getPublicUrl(nome);
        ...
        {anuncios.length === 0 ? (
          ...
          <div className="banner-list">
            {anuncios.map((a) => {
              ...
              return (
                <div className="banner-item" key={a.id}>
